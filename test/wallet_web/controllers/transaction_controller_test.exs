defmodule WalletWeb.TransactionControllerTest do
  ...
  setup do
    wallet1 = insert(:wallet, number: "0987654321")
    wallet2 = insert(:wallet, number: "1234567890")
  ...
  test "returns 200 on successful transfer", %{conn: conn, token: token} do
    conn = conn
    |> put_req_header("authorization", "Bearer #{token}")
    |> post("/api/transfer", %{"amount" => 100.0, "to_wallet_number" => 1234567890})
  ...
  test "returns 400 on insuficient found", %{conn: conn, token: token} do
    conn = conn
    |> put_req_header("authorization", "Bearer #{token}")
    |> post("/api/transfer", %{"amount" => 100.01, "to_wallet_number" => 1234567890})
